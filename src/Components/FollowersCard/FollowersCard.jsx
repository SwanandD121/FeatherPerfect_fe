  ...
  return (
    <div className="FollowersCard w-full p-5 rounded-lg flex flex-col gap-2 text-xs bg-white/70 dark:bg-slate-800">
      ...
      {Followers.map((follower, id) => {
        return (
          <div key={id} className="follower flex items-center justify-between">
            <div className="flex items-center gap-3">
              <img
                src={follower.img}
                alt=""
                className="followerImg rounded-full w-12 h-12 border-2 border-[#1060d7]"
              />
            ...
            <button
              ...
              onClick={onFollow} // Call the onFollow prop when clicked
            >
              Follow
            </button>
          </div>
